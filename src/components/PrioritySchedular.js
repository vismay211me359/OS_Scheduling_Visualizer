...
const PrioritySchedular = ({ processes, setCompletedProcesses, setGanttChartProcesses }) => {
    ...
    const interval = setInterval(() => {
      ...
      setReadyQueue((prevQueue) => {
        ...
        if (prevQueue.length === 0) {
          updatedQueue = [...prevQueue, ...newProcesses].sort((a, b) => {
          ...
          if (firstElement.burstTime === firstElement.initialBurst) {
            updatedQueue = [...prevQueue, ...newProcesses].sort((a, b) => {
              if (b.burstTime !== a.burstTime) {
                return a.priority - b.priority;
              }
  ...
  useEffect(() => {
    if (readyQueue.length > 0 && readyQueue[0].burstTime <= 0) {
      ...
      setReadyQueue((prevQueue) => prevQueue.slice(1).sort((a, b) => {
        if (b.burstTime !== a.burstTime) {
          return a.priority - b.priority;
        }
        return a.arrivalTime - b.arrivalTime;
...
export default PrioritySchedular
